...
/// The predicate is stored by [`ATTR_KEY_TILE_PRED`], with the same mapping as `cutile_ir`,
...
impl ConstantOp {
    pub fn new(ctx: &mut Context, res_ty: TypeHandle, rendered: &str) -> Self {
        ...
        op.get_operation().deref_mut(ctx).attributes.set(
            ATTR_KEY_TILE_VALUE.try_into().unwrap(),
...
pub fn set_binary(op: Ptr<Operation>, ctx: &Context, a: BinaryAttr) {
    op.deref_mut(ctx)
        .attributes
        .set(ATTR_KEY_TILE_OP.try_into().unwrap(), a);
}
...
pub fn set_cmp(op: Ptr<Operation>, ctx: &mut Context, kind: CmpKindAttr, pred: i64) {
    op.deref_mut(ctx)
        .attributes
        .set(ATTR_KEY_TILE_OP.try_into().unwrap(), kind);
    ...
    op.deref_mut(ctx)
        .attributes
        .set(ATTR_KEY_TILE_PRED.try_into().unwrap(), attr);
}
...
pub fn set_dim(op: Ptr<Operation>, ctx: &mut Context, dim: i64) {
    ...
    op.deref_mut(ctx)
        .attributes
        .set(ATTR_KEY_TILE_DIM.try_into().unwrap(), attr);
}
